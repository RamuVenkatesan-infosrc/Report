...
from app.models.improvement_models import DiscoveredAPI, SeverityLevel
...
class GitHubService:
        ...
        try:
            ...
            for api in discovered_apis:
                ...
                if (api.endpoint not in seen_endpoints and 
                    ...
                    not api.endpoint.startswith('ProjectModuleManager') or
                    not api.endpoint.startswith('Tega') or
                    not api.endpoint.startswith('infsvc') or
                    not api.endpoint.startswith('jira') or
                    not api.endpoint.startswith('testcase') or
                    not api.endpoint.startswith('builder') or
                    not api.endpoint.startswith('agent_history') or
                    not api.endpoint.startswith('history') or
                    not api.endpoint.startswith('agent') or
                    not api.endpoint.startswith('json') or
                    not api.endpoint.startswith('xml') or
                    not api.endpoint.startswith('yml') or
                    not api.endpoint.startswith('yaml') or
                    not api.endpoint.startswith('md') or
                    not api.endpoint.startswith('txt') or
                    not api.endpoint.startswith('log') or
                    not api.endpoint.startswith('lock') or
                    not api.endpoint.startswith('.idea') or
                    not api.endpoint.startswith('.vscode') or
                    not api.endpoint.startswith('node_modules') or
                    not api.endpoint.startswith('__pycache__') or
                    not api.endpoint.startswith('.git') or
                    not api.endpoint.startswith('venv') or
                    not api.endpoint.startswith('env') or
                    not api.endpoint.startswith('bin') or
                    not api.endpoint.startswith('obj') or
                    not api.endpoint.startswith('packages') or
                    not api.endpoint.startswith('target') or
                    not api.endpoint.startswith('build') or
                    not api.endpoint.startswith('dist') or
                    not api.endpoint.startswith('coverage') or
                    not api.endpoint.startswith('.pytest_cache') or
                    not api.endpoint.startswith('logs') or
                    not api.endpoint.startswith('test') or
                    not api.endpoint.startswith('tests') or
                    not api.endpoint.startswith('spec') or
                    not api.endpoint.startswith('specs') or
                    not api.endpoint.startswith('docs') or
                    not api.endpoint.startswith('documentation') or
                    not api.endpoint.startswith('assets') or
                    not api.endpoint.startswith('static') or
                    not api.endpoint.startswith('public') or
                    not api.endpoint.startswith('resources') or
                    not api.endpoint.startswith('config') or
                    not api.endpoint.startswith('.env') or
                    not api.endpoint.startswith('.gitignore') or
                    not api.endpoint.startswith('.gitattributes') or
                    not api.endpoint.startswith('package-lock') or
                    not api.endpoint.startswith('yarn.lock') or
                    not api.endpoint.startswith('composer.lock') or
                    not api.endpoint.startswith('pom.xml') or
                    not api.endpoint.startswith('build.gradle') or
                    not api.endpoint.startswith('requirements.txt') or
                    not api.endpoint.startswith('Pipfile') or
                    not api.endpoint.startswith('poetry.lock') or
                    len(api.endpoint) <= 3 or
                    api.endpoint.isdigit() or
                    api.endpoint.startswith('.') or
                    api.endpoint.endswith('.') or
                    api.endpoint.startswith(' ') or
    ...
    def _create_api_from_ai(self, api_data: dict, file_path: str, content: str) -> DiscoveredAPI:
        ...
        # Extract relevant code snippet - get full function definition
        lines = content.split('\n')
        # Use the improved extraction method to get complete function
        code_snippet = self._extract_code_snippet(lines, line_number - 1)  # Convert to 0-based index
    ...
    def _extract_code_snippet(self, lines: List[str], match_line: int) -> str:
        """Extract EXACT function/code block definition - no extra lines."""
        # Try to find the function/route definition start
        start_line = match_line
        end_line = match_line
        
        # Look backwards for decorators (Python) that come before the function
        decorator_lines = []
        for i in range(match_line, max(-1, match_line - 10), -1):
            if i < 0:
                break
            line = lines[i].strip()
            # Check for decorators like @app.route, @router.get, etc.
            if re.search(r'@.*\.(route|get|post|put|delete|patch|head|options)', line, re.IGNORECASE):
                decorator_lines.insert(0, i)
                start_line = i
            elif re.search(r'@(RequestMapping|GetMapping|PostMapping|PutMapping|DeleteMapping)', line, re.IGNORECASE):
                decorator_lines.insert(0, i)
                start_line = i
        
        # Search backwards for actual function definition
        for i in range(match_line, max(-1, match_line - 50), -1):
            if i < 0:
                break
            line = lines[i].strip()
            # Check for function definitions (not decorators)
            if re.search(r'^\s*(def|async def)\s+\w+\s*\(', line):
                # Found function definition - update start if no decorator found
                if not decorator_lines:
                    start_line = i
                break
            elif re.search(r'^\s*(function|const|let|var)\s+\w+', line) or re.search(r'^\s*export\s+(function|const)', line):
                # JavaScript/TypeScript function
                if not decorator_lines:
                    start_line = i
                break
            elif re.search(r'^\s*public\s+\w+\s+\w+\s*\(', line) or re.search(r'^\s*private\s+\w+\s+\w+\s*\(', line):
                # Java method
                if not decorator_lines:
                    start_line = i
                break
        ...
        if start_line < len(lines):
            # Get the base indentation of the function definition line
            func_def_line = lines[start_line] if start_line < len(lines) else ""
            base_indent = len(func_def_line) - len(func_def_line.lstrip())
            
            # If we have decorators, check their indentation too
            if decorator_lines:
                first_decorator = lines[decorator_lines[0]]
                decorator_indent = len(first_decorator) - len(first_decorator.lstrip())
                base_indent = min(base_indent, decorator_indent)
            
            # Find where function actually starts (after decorators/definition)
            func_body_start = start_line + 1
            for i in range(start_line, min(len(lines), start_line + 5)):
                if i >= len(lines):
                    break
                line = lines[i].strip()
                # Function body starts after definition line with colon or opening brace
                if i > start_line and (line.endswith(':') or line.endswith('{') or (line and '(' in line and (line.endswith(')') or '):' in line))):
                    func_body_start = i + 1
                    break
            
            # Find the exact end - look for next statement at same or less indentation
            end_line = func_body_start
            found_function_content = False
            brace_count = 0  # For languages with braces
            in_quotes = False
            quote_char = None
            
            # Check if this is a brace-based language (JavaScript/Java/C#)
            is_brace_language = False
            for check_idx in range(start_line, min(len(lines), start_line + 3)):
                if check_idx < len(lines) and '{' in lines[check_idx]:
                    is_brace_language = True
                    break
            
            for i in range(func_body_start, min(len(lines), start_line + 300)):
                if i >= len(lines):
                    end_line = i
                    break
                ...
                # Skip empty lines and comments (but count braces in comments for brace languages)
                if not line_stripped:
                    if found_function_content:
                        end_line = i + 1
                    continue
                
                # Track quotes to avoid counting braces in strings
                if not is_brace_language:
                    # Python-style: use indentation
                    current_indent = len(line) - len(line.lstrip())
                    
                    # Check if we've found actual function content
                    if current_indent > base_indent:
                        found_function_content = True
                        end_line = i + 1
                        continue
                    
                    # If we've been inside the function and found a line at base indent or less
                    if found_function_content:
                        if current_indent <= base_indent:
                            # This is likely the next function/statement - stop here
                            end_line = i
                            break
                    else:
                        # Still looking for function content
                        if current_indent > base_indent:
                            found_function_content = True
                            end_line = i + 1
                else:
                    # Brace-based language: track braces
                    found_function_content = True
                    for char in line:
                        if char in ('"', "'", '`'):
                            if not in_quotes:
                            ...
                            if char == '{':
                                ...
                                brace_count -= 1
                                if brace_count < 0:
                                    # Extra closing brace - we've gone too far
                                    end_line = i + 1
                                    break
                                elif brace_count == 0:
                                    # All braces closed - function ends here
                                    end_line = i + 1
                                    break
                    
                    if brace_count == 0 and found_function_content:
                        end_line = i + 1
                        break
                    else:
                        end_line = i + 1
        
        # Extract exact snippet - NO extra lines
        start = max(0, start_line)
        end = min(len(lines), end_line)  # No extra lines added
        
        # Remove any trailing empty lines from the selection
        while end > start and end < len(lines) and not lines[end - 1].strip():
            end -= 1
        
        snippet_lines = lines[start:end]
        code_snippet = '\n'.join(snippet_lines)
        
        # Clean up trailing empty lines from the result
        code_snippet = code_snippet.rstrip()
        
        # Log extraction for debugging
        logger.debug(f"Code snippet extracted: lines {start+1}-{end} ({end-start} lines, {len(code_snippet)} chars)")
        
        # If snippet is suspiciously short, verify we got the function
        if len(code_snippet.strip()) < 50:
            # Try a simpler approach - just get function definition + next 20 lines max
            if match_line < len(lines):
                simple_end = min(len(lines), start_line + 20)
                snippet_lines = lines[start_line:simple_end]
                code_snippet = '\n'.join(snippet_lines).rstrip()
                logger.debug(f"Using fallback extraction: lines {start_line+1}-{simple_end}")
